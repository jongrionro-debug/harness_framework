...
`harness_framework`는 지역 교육 프로그램을 운영하는 기관을 위한 운영 웹 MVP다.

조금 더 쉽게 말하면, 이 프로그램은 다음 일을 하나의 웹 안에서 연결한다.

1. 기관 운영자가 자기 기관용 운영 시스템을 만든다.
2. 운영자가 마을, 프로그램, 수업, 참여자, 강사를 등록한다.
...
운영자는 `/settings`에서 마을, 프로그램, 수업, 참여자를 추가한다.
...
즉 이 기능은 “조건부 활성화”다. 스토리지 설정이 없더라도 프로그램 전체가 깨지지 않고, 이유를 사용자에게 알려주는 방식으로 설계되어 있다.
