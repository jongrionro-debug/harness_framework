...
이 제품은 `지역 교육 사업 운영 시스템을 생성하고 운영하는 웹`이다.

다도리인을 첫 도입 기관으로 삼되, 제품 구조 자체는 특정 기관 전용이 아니라 `여러 기관/지역/마을이 각자 자기 사업을 설정해 사용할 수 있는 범용 운영 시스템`으로 설계한다.

즉, 1차 MVP의 목표는 두 가지를 동시에 만족하는 것이다.

1. 다도리인의 실제 운영 흐름을 빠르게 디지털화한다.
2. 이후 다른 기관도 인증 후 자기 기관 이름과 첫 번째 마을 이름을 입력해 운영 시스템을 생성하고, 그 안에서 `사업`, `수업`, `강사`, `참여자 명단` 등을 설정할 수 있게 만든다.
...
이 수정 가능 정책은 사업, 수업, 세션 등 하위 데이터가 생성된 이후에도 유지된다.

### 3. 사업

기관이 운영하는 교육 사업 또는 사업 단위다. 한 사업 아래 여러 수업이 있을 수 있다.

### 4. 수업

실제 반복 운영되는 수업 단위다. 수업은 특정 사업과 마을에 연결될 수 있다.
...
세션은 강사와 참여자 명단이 완전히 준비되기 전에도 생성할 수 있다. 운영자는 세션 생성 이후 세션별 관리 화면에서 강사를 할당하고 참여자를 추가한다. 단, 강사가 최종 제출을 하려면 세션 참여자 snapshot이 최소 1명 이상 있어야 한다.
...
운영자가 특정 세션에 참여자를 나중에 추가하는 경우에는 전체 참여자 명단에도 저장하고, 해당 세션 snapshot에도 추가한다. 제출 완료 후 추가된 참여자는 강사가 다시 수정 저장하기 전까지 출석 상태가 `미입력`으로 표시된다.
...
기관 생성 직후에는 사업, 수업, 강사, 참여자 명단이 비어 있는 상태를 허용한다. 이후 기관 운영자가 운영 기본정보 설정에서 하나씩 추가하는 흐름을 기본으로 한다.
...
- 자기 기관의 마을, 사업, 수업, 명단 등 운영 기본정보를 관리할 수 있다.
...
4. 생성 단계에서는 사업, 수업, 강사, 참여자 명단까지 한 번에 받지 않는다.
...
3. 다음 단계로 `사업`, `수업`, `강사`, `참여자 명단` 설정이 필요함을 안내해야 한다.
4. 사용자는 이 화면에서 운영 기본정보 설정으로 이동할 수 있어야 한다.
5. 기본 사업은 자동 생성하지 않으며, 사용자가 이후 직접 첫 사업을 만들 수 있어야 한다.
6. 이 화면의 가장 중요한 다음 액션은 `첫 사업 만들기`여야 한다.
...
2. 기관 운영자는 자기 기관의 사업과 수업을 생성/수정할 수 있어야 한다.
...
9. 사업이 하나도 없는 경우 `첫 사업 만들기`가 우선 액션으로 노출되어야 한다.
