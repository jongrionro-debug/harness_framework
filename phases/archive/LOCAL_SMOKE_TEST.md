...
1. `/settings`에서 사업을 1개 이상 만든다.
2. 수업을 1개 이상 만들고 사업/마을을 연결한다.
3. 참여자는 나중에 세션별로 추가해도 된다.

기대 결과:

- 비어 있는 상태에서도 첫 사업/첫 수업/첫 참여자 흐름이 막히지 않는다.
- 수업에 사업과 마을이 연결되지 않으면 이후 세션 생성에서 명확한 오류를 준다.

## 5. 사용자와 강사 준비

1. `/users`에서 초대 토큰을 만든다.
2. 같은 기관의 멤버 중 최소 1명을 `teacher` 역할로 둔다.
3. 필요한 경우 `teacher` 역할 사용자를 수업에 배정한다.
...
1. `/dashboard`에서 날짜, 마을, 사업, 수업을 선택하고 강사는 비워 둔 채 세션을 만든다.
2. 세션 생성 후 최근 생성 세션에 바로 보이는지 확인한다.
3. 최근 생성 세션에서 `세션 관리`를 연다.
4. 세션 관리 화면에서 강사를 할당하고 참여자를 추가한다.

기대 결과:

- `sessions` row가 생성된다.
- 강사와 참여자가 없어도 세션이 생성된다.
- 참여자 추가 시 전체 명단과 `session_participant_snapshots`가 함께 갱신된다.
...
5. 운영자 계정으로 돌아와 제출 완료 세션의 관리 화면에서 참여자를 1명 더 추가한다.
6. records 상세에서 새 참여자가 `미입력`으로 보이는지 확인한다.
7. 강사 계정으로 다시 열어 수정 저장한다.
...
- 제출 후 추가된 참여자는 강사 수정 저장 전까지 출석 상태가 `미입력`이다.
...
2. 다음 영역이 구분되어 보이는지 확인한다.
   ...
   - 최근 수정
   - 미제출 세션
3. `/records`에서 검색/상태/사업/강사 필터를 바꿔 특정 세션을 찾는다.
